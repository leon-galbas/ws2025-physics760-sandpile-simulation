...
class CaptureSandpileModel:
    ...
    def __init__(
        ...
        self._r_0 = None
        self.sandpile_time_series=torch.empty(capture_time,N,N, dtype=torch.int64)
        self._micro_time = 0
        # init z tensor
        z_shape = (N,) * d
    ...
    def relax(self, capture= True) -> tuple[int, int, int]:
    ...
    def burn_in(
        ...
        while True:
            _, _, _ = self.relax(capture=False)
            self.perturb(capture=False)
